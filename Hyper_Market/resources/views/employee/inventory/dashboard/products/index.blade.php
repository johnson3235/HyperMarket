        ...
        <div class="card">
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped text-center">
                    ...
                    <tbody>
                        {{--  <tr>
                            @foreach (Auth::user()->notifications as $nofication )
                                <th>{{$nofication->data['name']  }}</th>
                            @endforeach
                        </tr>  --}}
                        @foreach ($products as $product)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form action="{{route('employee.products.toggle',$product->id)}}" method="post" class="form" >
                                        @csrf
                                        @method('PATCH')
                                        <input type="checkbox" name="my-checkbox" {{$product->status ? 'checked' : ''}}   data-off-color="danger" data-on-color="success" value= "{{$product->status}}" data-bootstrap-switch  >
